...
interface Callback<error, value = void> {
...
// Technically Duplex can consist of `Source<rx, ra>` and `Sink<wx, wa>` where
// data read is of different type from data being written & so are errors that
// could occur. But in most cases same data type is both read and written, there
// for Sink types parameters `wx` and `wa` are optional and match read type
// parameters if not specified. You could also specify tree type parameters in
// case errors are different but data is read and wrote is the same.
interface Duplex<rx, ra, wx = rx, wa = ra> {
  source: Source<rx, ra>;
  sink: Sink<wx, wa>;
}
...
declare module "pull-stream" {
  declare export type Source<x, a> = Source<x, a>
  declare export type Through<x1, x2, a1, a2> = Through<x1, x2, a1, a2>
  declare export type Sink<x, a> = Sink<x, a>
  declare export type Duplex<rx, ra, wx = rx, wa = ra> = Duplex<rx, ra, wx, wa>
